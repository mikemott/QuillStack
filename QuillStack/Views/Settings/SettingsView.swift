...
struct SettingsView: View {
    ...
    @State private var remoteHost = UserDefaults.standard.string(forKey: "remoteServerHost") ?? ""
    ...
    private var ocrSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 0) {
                settingsInput("Remote Server IP", text: $remoteHost, key: "remoteServerHost")
            ...
            Text("Enter your remote server's Tailscale IP address. Captures queue when offline.")
                .font(QSFont.monoLight(size: 11))
                .foregroundStyle(QSColor.onSurfaceMuted)
                .padding(.leading, 4)
        }
        ...
        HStack {
            ...
            TextField("", text: text)
                ...
                .onChange(of: text.wrappedValue) { _, newValue in
                    ...
                    if key == "remoteServerHost" {
                        Task {
                            await RemoteOCRService.shared.setRemoteHost(newValue)
                            await checkConnection()
                        }
                    }
                }
